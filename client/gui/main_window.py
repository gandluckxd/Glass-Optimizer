...
"""
    ...
    def _handle_optimization_result(self, result):
        ...
        # ПРОВЕРКА: Если оптимизация не удалась (недостаточно материала)
        if not result.success:
            # Показываем сообщение об ошибке пользователю
            print(f"❌ Оптимизация не удалась: {result.message}")
            
            # Создаем красивое диалоговое окно с ошибкой
            msg_box = QMessageBox(self)
            msg_box.setWindowTitle("❌ Недостаточно материала")
            msg_box.setIcon(QMessageBox.Critical)
            
            # Основной текст
            if "НЕДОСТАТОЧНО МАТЕРИАЛА" in result.message:
                msg_box.setText("НЕДОСТАТОЧНО МАТЕРИАЛА ДЛЯ ВЫПОЛНЕНИЯ РАСКРОЯ")
            else:
                msg_box.setText("Оптимизация не может быть выполнена")
            
            # Детальная информация
            msg_box.setDetailedText(result.message)
            msg_box.setInformativeText(
                "Программа не может разместить все детали из-за нехватки материала.\n\n"
                "Нажмите 'Show Details' для получения подробной информации о недостающих материалах."
            )
            
            # Показываем диалог
            msg_box.exec_()
            
            # Обновляем статистику (чтобы показать частичный результат, если есть)
            if result.layouts:
